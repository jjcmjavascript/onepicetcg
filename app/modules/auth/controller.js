...
class AuthController {
  ...
  async login(req, res) {
    try {
      ...
      const userFound = await this.dbUser.findOne({
        where: {
          email: email,
        },
      });

      if (!userFound || !this.comparePassword(password, userFound.password)) {
        return res
          .status(401)
  ...
  async getAll(req, res) {
    const users = await this.dbUser.findAll().catch((err) => {
      return res.status(500).json({
